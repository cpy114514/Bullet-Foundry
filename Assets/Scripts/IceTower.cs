...
public sealed class IceTower : MonoBehaviour
{
    [SerializeField]
    private Sprite[] iceFrames = new Sprite[4];

    [SerializeField, Min(0.01f)]
    private float frameDuration = 0.08f;

    [SerializeField]
    private Vector2 iceBulletScale = new Vector2(0.75f, 0.75f);
    ...
    private void TryConvert(Bullet bullet)
    {
        ...
        if (bullet.Element == BulletElement.Fire)
        {
        ...
        if (bullet.Element == BulletElement.Normal)
        {
            bullet.ApplyElement(
                ...
                frameDuration,
                iceBulletScale);
            bullet.SpawnConversionEffect(BulletElement.Ice);
        }
    }
